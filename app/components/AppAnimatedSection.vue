...
function initializeAnimation() {
  ...
  // Animations will be started by intersection observer when in viewport
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (isInViewport) {
          ...
          // Stop animations when leaving viewport
          if (animationId) {
            cancelAnimationFrame(animationId);
            animationId = null;
          }
...
function animateGrid() {
  // Only continue animation if in viewport
  if (!isInViewport) {
    animationId = null;
    return;
  }
